...
def get_child_ids(hwnd):
    ...
    while child != 0:
        if child_name:
            child_ids[child_name] = child
        else:
            child_ids[f"Child {v}"] = child
...
class Taskbar:
    def __init__(self):
        # hWnd
        self.hTaskbar = win32gui.FindWindow("Shell_TrayWnd", None)
        self.idsnames = get_child_ids(self.hTaskbar)
        print(self.idsnames)
        self.hTray = self.idsnames["Child 4"]
        self.hStartbtn = self.idsnames["Start"]
        ...
        # Rects
        self.tx, self.ty, self.twidth, self.theight = win32gui.GetWindowRect(
            self.hTaskbar
        )
    ...
    def Rounded_Taskbar(self):
        ...
        width = (
            win32gui.GetWindowRect(self.hTaskbar)[2]
            - win32gui.GetWindowRect(self.hTaskbar)[0]
        )
        height = (
            win32gui.GetWindowRect(self.hTaskbar)[3]
            - win32gui.GetWindowRect(self.hTaskbar)[1]
        )
        width = (
            win32gui.GetWindowRect(self.hTaskbar)[2]
            - win32gui.GetWindowRect(self.hTaskbar)[0]
        ...
        rect = win32gui.GetWindowRect(self.idsnames["Child 2"])
        sx, sy, sw, sh = self.hstartMrect
        x, y, w, h = rect
        win32gui.SetWindowRgn(
            self.hTaskbar, win32gui.CreateRoundRectRgn(0, 0, width, height, 0, 0), True
        )
        rgn = win32gui.CreateRoundRectRgn(
            width // 2 - sx + 20, 1, width // 2 + sx, height, 15, 15
        )
